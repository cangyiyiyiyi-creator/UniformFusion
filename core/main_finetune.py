...
def resolve_attention_config(name):
    ATTN_MAP = {
        ...
        # W2: weak-class granularity modulation on N3, weak-class proto modulation on N4,
        # no attention on N5. This is the most promising structure for follow-up work.
        ...
        # N3 keeps stable Granularity, N4 uses difficulty-aware weak-class expert routing,
        # no attention on N5
        "dwr_gran_dwr_none": {
            "N3": "granularity",
            "N4": "dwr_route",
...
def get_args_parser():
    ...
    parser.add_argument('--fuse_mode', default='concat', choices=['concat', 'add', 'gated', 'xattn', 'ahcr', 'cv_gsc_add'])
    ...
    # vision-semantic multi-modal auxiliary branch: off by default; when enabled it leaves the
    # original R2 classification head untouched and only adds a small-weight fusion of semantic logits
...
def main(args):
    ...
    if getattr(args, "plain_innovation_freeze_base", False):
        if enabled_innovations not in (
            ["use_p16_facgr"], ["use_p17_dcasr"], ["use_p19_apcer"]
        ):
            raise ValueError(
                "plain_innovation_freeze_base requires P16, P17, or P19"
            )
        if not getattr(args, "finetune", ""):
            raise ValueError(
                "frozen P16/P17/P19 requires --finetune with a Plain-BCE checkpoint"
            )
    ...
    # The GSPF wrapper must always stay enabled: even with lambda=0 the GSPF cache has to be
    # cleared after every batch, otherwise GPU memory keeps growing.
    ...
    if args.patience > 0:
        ...
        if args.early_stop_start_epoch > 0:
            print(
                f"[main] early-stop counting starts at epoch {args.early_stop_start_epoch}"
            )
