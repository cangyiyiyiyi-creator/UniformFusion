...
def main() -> None:
    ...
    pattern = "resnet50/seed_*/repeat_*/AHCR_Uniform/val_metrics.json"
    for val_path in sorted(run_root.glob(pattern)):
        test_path = val_path.with_name("test_metrics.json")
        if not test_path.is_file():
            raise FileNotFoundError(f"missing test metrics: {test_path}")
    ...
    expected_methods = {"Plain_BCE", "Final_NoAnchor", UNIFORM_METHOD}
    ...
    detailed_path = output_root / "ahcr_uniform_n5_detailed.csv"
    ...
    summary_rows = []
    for method in ("Plain_BCE", UNIFORM_METHOD, "Final_NoAnchor"):
        ...
        delta_plain = [float(row["test_mAP"]) - plain[int(row["seed"])] for row in selected]
        delta_final = [float(row["test_mAP"]) - final[int(row["seed"])] for row in selected]
        summary_rows.append(
            {
                "method": method,
                "n": len(selected),
    ...
    summary_path = output_root / "ahcr_uniform_n5_summary.csv"
    with summary_path.open("w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(summary_rows[0]))
        writer.writeheader()
        writer.writerows(summary_rows)
    print(f"OFFICIAL_AHCR_N5_SUMMARY_OK rows={len(rows)} output={output_root}")
