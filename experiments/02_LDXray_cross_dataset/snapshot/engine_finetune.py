...
def _official_ahcr_supervised_loss(model_out, criterion, target: th.Tensor):
    if not isinstance(model_out, dict):
        return None
    branch_logits = model_out.get("official_ahcr_branch_logits")
    if branch_logits is None:
        return None
    if not isinstance(criterion, nn.BCEWithLogitsLoss):
        raise TypeError("official AHCR fair reproduction requires BCEWithLogitsLoss")
    if not isinstance(branch_logits, (tuple, list)) or len(branch_logits) != 2:
        raise ValueError("official AHCR must provide OL and SD branch logits")
    ol_logits, sd_logits = branch_logits
    return 0.5 * (criterion(ol_logits, target) + criterion(sd_logits, target))
...
def _plain_bce_innovation_loss(model_out, target: th.Tensor):
    ...
    router_variant = str(aux.get("router_variant", "legacy"))
    if (
        mode == "p9_caprs"
        and router_variant == "selective_regret"
        and expert_logits is not None
        and aux.get("utility_logits") is not None
    ):
        utility_logits = aux["utility_logits"].float()
        expanded_target = target.unsqueeze(-1).expand_as(expert_logits)
        ...
        with th.no_grad():
            ...
            candidates = expert_logits[..., 1:].detach().float()
            base_detached = base.detach().float()
            # The utility label measures the signed regret reduction attainable
            # through the configured fixed-gamma bounded correction.
            gamma = aux.get("probe_gamma")
            if gamma is None:
                gamma = aux.get("gamma")
            if gamma is None:
                gamma = zero.new_tensor(0.05)
            gamma = gamma.detach().float()
            probe_logits = base_detached.unsqueeze(-1) + (
                gamma
                * 4.0
                * th.tanh(
                    (candidates - base_detached.unsqueeze(-1)) / 4.0
                )
            )
            probe_target = target.unsqueeze(-1).expand_as(probe_logits)
            probe_element = F.binary_cross_entropy_with_logits(
                probe_logits, probe_target, reduction="none"
            )
            signed_gain = base_element.unsqueeze(-1) - probe_element
            margin = aux.get("gain_margin")
            if margin is None:
                margin = zero.new_tensor(1e-5)
            margin = margin.detach().float().clamp_min(0.0)
            utility_target = (signed_gain > margin).float()

            # Each class/expert binary task contributes equal positive and
            # negative mass, avoiding frequent classes or experts dominating.
            positive_fraction = utility_target.mean(dim=0).clamp(0.05, 0.95)
            utility_weight = th.where(
                utility_target > 0.5,
                0.5 / positive_fraction.unsqueeze(0),
                0.5 / (1.0 - positive_fraction).unsqueeze(0),
            )
            any_helpful = utility_target.any(dim=-1)

        utility_element = F.binary_cross_entropy_with_logits(
            utility_logits, utility_target, reduction="none"
        )
        utility_loss = (
            utility_element * utility_weight
        ).sum() / utility_weight.sum().clamp_min(1.0)

        pair_elements = []
        pair_masks = []
        for left, right in ((0, 1), (0, 2), (1, 2)):
            gain_difference = signed_gain[..., left] - signed_gain[..., right]
            pair_mask = any_helpful & (gain_difference.abs() > margin)
            pair_target = (gain_difference > 0.0).float()
            pair_element = F.binary_cross_entropy_with_logits(
                utility_logits[..., left] - utility_logits[..., right],
                pair_target,
                reduction="none",
            )
            pair_elements.append(pair_element)
            pair_masks.append(pair_mask.float())
        pair_element = th.stack(pair_elements, dim=-1)
        pair_mask = th.stack(pair_masks, dim=-1)
        pairwise_loss = (
            pair_element * pair_mask
        ).sum() / pair_mask.sum().clamp_min(1.0)
        route_loss = utility_loss + 0.5 * pairwise_loss
        route_active = any_helpful.float().mean()
    elif (
        mode == "p9_caprs"
        and router_variant == "residual_nonbase"
        and expert_logits is not None
        and aux.get("gate_logits") is not None
        and aux.get("nonbase_router_logits") is not None
    ):
        gate_logits = aux["gate_logits"].float()
        nonbase_router_logits = aux["nonbase_router_logits"].float()
        expanded_target = target.unsqueeze(-1).expand_as(expert_logits)
        expert_element = F.binary_cross_entropy_with_logits(
            expert_logits.float(), expanded_target, reduction="none"
        )
        with th.no_grad():
            base_element = expert_element[..., 0]
            nonbase_element = expert_element[..., 1:]
            raw_positive_gain = F.relu(
                base_element.unsqueeze(-1) - nonbase_element
            )

            # Gate supervision follows the improvement that is actually
            # attainable under the configured bounded residual budget.  The
            # probe omits the learned gate/ramp so routing can learn from
            # epoch zero without leaking gradients into any expert.
            gamma = aux.get("gamma")
            if gamma is None:
                gamma = zero.new_tensor(0.05)
            gamma = gamma.detach().float()
            nonbase_experts = expert_logits[..., 1:].detach().float()
            base_detached = base.detach().float()
            probe_logits = base_detached.unsqueeze(-1) + (
                gamma
                * 4.0
                * th.tanh(
                    (nonbase_experts - base_detached.unsqueeze(-1)) / 4.0
                )
            )
            probe_target = target.unsqueeze(-1).expand_as(probe_logits)
            probe_element = F.binary_cross_entropy_with_logits(
            ...
            probe_gain = (
                base_element.unsqueeze(-1) - probe_element
            ).clamp_min(0.0)
            best_probe_gain = probe_gain.max(dim=-1).values
            gate_target = (best_probe_gain > 1e-7).float()

            temperature = aux.get("advantage_temperature")
            if temperature is None:
                temperature = zero.new_tensor(0.05)
            temperature = temperature.detach().float().clamp_min(1e-4)
            expert_target = F.softmax(
                raw_positive_gain / temperature, dim=-1
            )

            positive_fraction = gate_target.mean(dim=0).clamp(0.05, 0.95)
            class_balance = th.where(
                gate_target > 0.5,
                0.5 / positive_fraction.unsqueeze(0),
                0.5 / (1.0 - positive_fraction).unsqueeze(0),
            )
            positive_mean = (
                best_probe_gain.sum() / gate_target.sum().clamp_min(1.0)
            ).clamp_min(1e-7)
            magnitude = th.where(
                gate_target > 0.5,
                (best_probe_gain / positive_mean).clamp(0.25, 4.0),
                th.ones_like(best_probe_gain),
            )
            gate_weight = class_balance * magnitude
        ...
        expert_element_loss = -(
            expert_target
            * F.log_softmax(nonbase_router_logits, dim=-1)
        ).sum(dim=-1)
        expert_loss = (
            expert_element_loss * gate_target
        ).sum() / gate_target.sum().clamp_min(1.0)
        route_loss = gate_loss + expert_loss
        route_active = gate_target.mean()
    elif mode == "p17_dcasr" and router_logits is not None:
...
def train_one_epoch(
    ...
    for samples in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with th.autocast(device_type='cuda', dtype=ac_dtype, enabled=amp):
            ...
            official_ahcr_loss = _official_ahcr_supervised_loss(
                model_out, plain_criterion, target
            )
            loss_sup = (
                official_ahcr_loss
                if official_ahcr_loss is not None
                else plain_criterion(logits, target)
            )

            if official_ahcr_loss is not None:
                if is_distill:
                    raise RuntimeError("official AHCR reproduction does not support distillation")
                ...
                if any(float(getattr(criterion, name, 0.0)) != 0.0 for name in regularization_names):
                    raise RuntimeError(
                        "official AHCR reproduction requires all project regularizers disabled"
                    )
                total_loss = official_ahcr_loss
            elif is_distill:
