...
class P9CAPRSHead(ScheduledInnovationHead):
    ...
    def __init__(
        ...
        self.router_variant = str(router_variant).lower()
        if self.router_variant not in (
            "legacy",
            "residual_nonbase",
            "selective_regret",
        ):
            raise ValueError(
                "P9 router_variant must be legacy, residual_nonbase, or "
                "selective_regret; "
                f"got {router_variant!r}"
            )
        if self.router_variant in ("residual_nonbase", "selective_regret"):
            if not self.use_counterfactual_experts:
                raise ValueError(
                    f"P9 {self.router_variant} requires counterfactual experts"
                )
            if not self.use_learned_router:
                raise ValueError(
                    f"P9 {self.router_variant} requires a learned router"
                )
            if float(base_floor) != 0.0:
                raise ValueError(
                    f"P9 {self.router_variant} requires base_floor=0 because "
                    "its gate already preserves the base expert"
                )
        self.gamma_trainable = bool(gamma_trainable)
        self.gamma_initial = max(float(gamma_init), 0.0)
        self.gamma_max = max(float(gamma_max), 1e-8)
        if self.gamma_initial > self.gamma_max:
            raise ValueError(
                "P9 gamma_init cannot exceed gamma_max: "
                f"{self.gamma_initial} > {self.gamma_max}"
            )
        self.gate_init = min(max(float(gate_init), 1e-4), 1.0 - 1e-4)
        self.advantage_temperature = max(
            float(advantage_temperature), 1e-4
        )
        self.gain_margin = float(gain_margin)
        if self.gain_margin < 0.0:
            raise ValueError("P9 gain_margin must be non-negative")
        self.score_temperature = float(score_temperature)
        if self.score_temperature <= 0.0:
            raise ValueError("P9 score_temperature must be positive")
        self._router_only = False
        ...
        if self.router_variant == "selective_regret":
            # Keep the legacy router trunk input-compatible so its learned
            # representation can be restored from a Final checkpoint.
            router_experts = 3
        nn.init.zeros_(self.pair_expert[-1].weight)
        nn.init.zeros_(self.pair_expert[-1].bias)
        self.router = None
        self.direction_adapter = None
        if self.use_learned_router:
            ...
            self._initialize_router_output()
            if self.router_variant == "selective_regret":
                self.direction_adapter = nn.Linear(4, 3, bias=False)
                nn.init.zeros_(self.direction_adapter.weight)
        self.gamma_raw = _bounded_parameter(gamma_init, self.gamma_max)
        self.gamma_raw.requires_grad_(self.gamma_trainable)

    def _initialize_router_output(self) -> None:
        if self.router is None:
            return
        output = self.router[-1]
        nn.init.zeros_(output.weight)
        if self.router_variant == "residual_nonbase":
            prior = output.bias.new_zeros(output.out_features)
            prior[0] = _probability_logit(self.gate_init)
        elif self.router_variant == "selective_regret":
            # Equal utility priors imply q=1/3 while log-mean-exp maps the
            # shared prior exactly back to gate_init.
            prior = output.bias.new_full(
                (output.out_features,), _probability_logit(self.gate_init)
            )
        else:
            probabilities = (
                [0.80, 0.07, 0.07, 0.06]
            ...
            prior = output.bias.new_tensor(probabilities).log()
        with torch.no_grad():
            output.bias.copy_(prior)

    def reset_router_parameters(self) -> None:
        """Reset every router parameter after loading an older checkpoint."""
        if self.router is None:
            raise RuntimeError("P9 router reset requires a learned router")
        for module in self.router.modules():
            if isinstance(module, (nn.Linear, nn.LayerNorm)):
                module.reset_parameters()
        self._initialize_router_output()
        if self.direction_adapter is not None:
            nn.init.zeros_(self.direction_adapter.weight)

    def transfer_legacy_router_output(
        self,
        legacy_weight: torch.Tensor,
        legacy_bias: torch.Tensor,
    ) -> None:
        """Map legacy non-base rows 1:4 into selective utility outputs."""
        if self.router_variant != "selective_regret" or self.router is None:
            raise RuntimeError(
                "legacy router transfer requires P9 selective_regret"
            )
        output = self.router[-1]
        expected_weight = (4, output.in_features)
        if tuple(legacy_weight.shape) != expected_weight:
            raise ValueError(
                "legacy router output weight has incompatible shape: "
                f"expected {expected_weight}, got {tuple(legacy_weight.shape)}"
            )
        if tuple(legacy_bias.shape) != (4,):
            raise ValueError(
                "legacy router output bias has incompatible shape: "
                f"expected (4,), got {tuple(legacy_bias.shape)}"
            )
        with torch.no_grad():
            output.weight.copy_(legacy_weight[1:4].to(output.weight))
            output.bias.copy_(legacy_bias[1:4].to(output.bias))
            if self.direction_adapter is not None:
                self.direction_adapter.weight.zero_()

    def set_router_only(self) -> None:
        if self.router_variant not in ("residual_nonbase", "selective_regret"):
            raise RuntimeError(
                "P9 router-only training requires residual_nonbase or "
                "selective_regret"
            )
        if self.router is None:
            raise RuntimeError("P9 router-only training requires a router")
        for parameter in self.parameters():
            parameter.requires_grad = False
        for parameter in self.router.parameters():
            parameter.requires_grad = True
        if self.direction_adapter is not None:
            for parameter in self.direction_adapter.parameters():
                parameter.requires_grad = True
        self._router_only = True
        self.train(self.training)

    def train(self, mode: bool = True):
        super().train(mode)
        if mode and self._router_only:
            self.encoder.eval()
            self.region_classifier.eval()
            self.pair_expert.eval()
            self.router.train(True)
            if self.direction_adapter is not None:
                self.direction_adapter.train(True)
        return self
    ...
    def forward(
        ...
        if self.router_variant == "selective_regret":
            base_detached = logits_base.detach()
            signed_candidate_delta = (
                experts[..., 1:].detach() - base_detached.unsqueeze(-1)
            )
            direction_stats = torch.cat(
                [
                    torch.tanh(base_detached / 4.0).unsqueeze(-1),
                    torch.tanh(signed_candidate_delta / 4.0),
                ],
                dim=-1,
            ).to(dtype=stats.dtype)
        gate_logits = None
        nonbase_router_logits = None
        utility_logits = None
        if self.use_learned_router:
            router_input = torch.cat([paired, stats], dim=-1)
            if self.router_variant == "selective_regret":
                router_input = router_input.detach()
            router_logits = self.router(router_input)
            if self.router_variant == "residual_nonbase":
                gate_logits = router_logits[..., 0]
                nonbase_router_logits = router_logits[..., 1:]
                residual_gate = torch.sigmoid(gate_logits)
                nonbase_weights = nonbase_router_logits.softmax(dim=-1)
                learned_weights = torch.cat(
                    [
                        (1.0 - residual_gate).unsqueeze(-1),
                        residual_gate.unsqueeze(-1) * nonbase_weights,
                    ],
                    dim=-1,
                )
            elif self.router_variant == "selective_regret":
                utility_logits = router_logits + self.direction_adapter(
                    direction_stats.detach()
                )
                router_logits = utility_logits
                nonbase_weights = F.softmax(
                    utility_logits / self.score_temperature, dim=-1
                )
                gate_logits = (
                    torch.logsumexp(utility_logits, dim=-1) - math.log(3.0)
                )
                residual_gate = torch.sigmoid(gate_logits)
                learned_weights = torch.cat(
                    [
        ...
        if self.gamma_trainable:
            ...
            gamma = logits_base.new_tensor(self.gamma_initial)
        ...
        return {
            ...
            "nonbase_router_logits": nonbase_router_logits,
            "utility_logits": utility_logits,
            "gain_margin": logits_base.new_tensor(self.gain_margin),
            "score_temperature": logits_base.new_tensor(
                self.score_temperature
            ),
            "advantage_temperature": logits_base.new_tensor(
            ...
            "gamma": gamma,
            "probe_gamma": logits_base.new_tensor(self.gamma_initial),
            "ramp": ramp,
        }
