...
from math import inf  # PyTorch 2.x: replace torch._six.inf
import argparse
...
class SmoothedValue(object):
    ...
    @property
    def max(self):
        return max(self.deque) if len(self.deque) else 0.0

    @property
    def value(self):
        return self.deque[-1] if len(self.deque) else 0.0
...
def init_distributed_mode(args):
    if getattr(args, 'dist_on_itp', False):
        ...
        os.environ['RANK'] = str(args.rank)
        os.environ['LOCAL_RANK'] = str(args.gpu)
        os.environ['WORLD_SIZE'] = str(getattr(args, 'world_size', 1))
    else:
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if getattr(args, 'auto_resume', False) and len(getattr(args, 'resume', '')) == 0:
        ...
        print("Auto resume checkpoint: %s" % getattr(args, 'resume', ''))
...
# --- compat: print only on rank-0 (fallback to always print if no dist) ---
def print_on_master(*args, **kwargs):
    """
    Safe printing utility used by engine_finetune.py.
    If distributed is available & initialized, only rank-0 prints;
