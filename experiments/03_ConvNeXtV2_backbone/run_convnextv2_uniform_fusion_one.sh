...
[[ -f "${FINETUNE}" ]] || { echo "Missing pretrained weights: ${FINETUNE}"; exit 2; }
[[ "$(realpath "${VAL_LIST}")" != "$(realpath "${TEST_LIST}")" ]] || {
  echo "VAL_LIST and TEST_LIST must differ"; exit 2;
}

METHOD="Uniform_Fusion"
OUT_DIR="${SAVE_ROOT}/convnextv2_tiny/seed_${SEED}/repeat_${REPEAT}/${METHOD}"
mkdir -p "${OUT_DIR}"
...
command=(
  ...
  --model convnextv2_tiny --model_prefix "" --finetune "${FINETUNE}"
...
evaluation_command() {
  ...
  printf '%q ' env CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" -u \
    ...
    --classes-file "${CLASSES_FILE}" --view-mode paired \
    --output-json "${OUT_DIR}/${split_name}_metrics.json" \
    --output-csv "${OUT_DIR}/${split_name}_metrics.csv" \
...
if [[ "${DRY_RUN,,}" == "true" ]]; then
  ...
  evaluation_command "${PHASE}" "$([[ "${PHASE}" == "val" ]] && printf '%s' "${VAL_LIST}" || printf '%s' "${TEST_LIST}")"
  exit 0
fi

if [[ "${PHASE}" == "val" && ! -f "${training_marker}" ]]; then
  echo "[$(date '+%F %T')] TRAIN START convnextv2_tiny ${METHOD} seed=${SEED}"
  ...
  echo "[$(date '+%F %T')] TRAIN DONE convnextv2_tiny ${METHOD} seed=${SEED}"
elif [[ "${PHASE}" == "val" ]]; then
  echo "[SKIP TRAIN] convnextv2_tiny ${METHOD} seed=${SEED}"
fi
...
list_path="${VAL_LIST}"
[[ "${PHASE}" == "test" ]] && list_path="${TEST_LIST}"
output_json="${OUT_DIR}/${PHASE}_metrics.json"
output_csv="${OUT_DIR}/${PHASE}_metrics.csv"
if [[ -s "${output_json}" && -s "${output_csv}" ]]; then
  echo "[SKIP ${PHASE^^}] convnextv2_tiny ${METHOD} seed=${SEED}"
  exit 0
fi
if [[ -e "${output_json}" || -e "${output_csv}" ]]; then
  echo "Incomplete ${PHASE} output: ${OUT_DIR}"; exit 1
fi

echo "[$(date '+%F %T')] ${PHASE^^} START convnextv2_tiny ${METHOD} seed=${SEED}"
CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" -u \
  ...
  --batch-size "${BATCH_SIZE}" --num-workers "${NUM_WORKERS}" --device cuda
echo "[$(date '+%F %T')] ${PHASE^^} DONE convnextv2_tiny ${METHOD} seed=${SEED}"
